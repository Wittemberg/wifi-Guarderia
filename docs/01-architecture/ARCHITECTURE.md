...
```mermaid
flowchart TD
  PC[Computador administrativo] --> CASA[RB951G em casa]
  CASA -->|WireGuard iniciado de dentro do CGNAT| VPS[VPS Ubuntu: WireGuard no host]
  ...
  VPS --> NOC[Docker: Zabbix, PostgreSQL, Grafana, Kuma]
```
...
| RB951G | Roteamento da estação administrativa para a VPN | Internet residencial |
...
2. **Gerência doméstica:** PC administrativo → RB951 → WireGuard → VPS → WireGuard → core → equipamento. Rotas de retorno são obrigatórias.
...
|---|---|---|
| Internet de casa | Guarderia e NOC | Administração por casa |
